"""
Interactive Demo Page - M&A Classifier
Provides text input, PDF upload, and sample testing
...
def main():
    st.set_page_config(
        page_title="Interactive Demo - M&A Classifier",
        page_icon="🔬",
        layout="wide"
    )
    
    st.title("🔬 Interactive Demo")
    st.markdown("Test the M&A classifier with your own data or pre-loaded samples")
    
    st.markdown("---")
    
    # Sidebar - Information
    with st.sidebar:
        st.header("ℹ️ About the Classifier")
        st.markdown("""
